...
class ApplicationController {
    ...
    public async applyForJob(req: Request, res: Response) {
        try{
            ...
            if (existingApp) return res.status(400).send("Already applied to this job");
    ...
    public async acceptApplication(req: Request, res: Response) {
        try {
            ...
            if (!assignment) return res.status(404).send("Application not found");
            ...
            await Application.updateMany(
                { jobId: assignment.jobId, _id: { $ne: assignment._id } },
                { $set: { status: "rejected" } }
            );
     ...
     */
    public async rejectApplication(req: Request, res: Response) {
        try {
            if(req.user?.role !== "NGO"){
                return res.status(401).send("Unauthorized");
            }
