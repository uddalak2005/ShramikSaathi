...
import type {bidModel} from "../models/bid.model.ts";
import mongoose from "mongoose";

class BidController {
    // Create or Update Bid
    public async placeOrUpdateBid(req: Request, res: Response) {
        try {
            ...
            if (bid) {
                // Update existing bid
                const oldWage : bidModel = {
                    bidType: req.user?.role as string,
    ...
    public async acceptBid(req: Request, res: Response) {
        try {
    ...
    public async rejectBid(req: Request, res: Response) {
        try {
            const { bidId } = req.params;
